...
if [ $Allelecode = 12 ]; then
 ...
 ./plink --silent --cow --noweb --nonfounders --bfile ../${ref} --make-bed --out ref_upd
...
rm recodeallele.txt
 ...
 ./plink --silent --cow --noweb --nonfounders --bfile ../${ref} --update-alleles alleleupdate.txt --make-bed --out ref_upd
...
thresmm=$(awk 'END {print 0.01}' ../${ref}.bim)
thresm=$(awk 'END {print 0.0008}' ../${ref}.bim)
echo "title=*population based imputation of ${finaloutfile}*;
genotype_file=*${outref}.geno*;
snp_info_file=*${outref}.snpinfo*;
ped_file=*../${Pedigree}*;
...
if [ $Allelecode = 12 ]; then
 ./plink --silent --cow --noweb --nonfounders --file file --make-bed --out ${finaloutfile}_imp
 ./plink --silent --cow --noweb --nonfounders --bfile ${finaloutfile}_imp --make-bed --out ${finaloutfile}_imp
rm file.map file.ped
elif [ $Allelecode = AB ]; then
 cat ../${ref}.bim | awk '{print $2,1,2,"A","B"}' > alleleupdate.txt
 ./plink --silent --cow --noweb --nonfounders --file file --update-alleles alleleupdate.txt --make-bed --out ${finaloutfile}_imp
 ./plink --silent --cow --noweb --nonfounders --bfile ${finaloutfile}_imp --make-bed --out ${finaloutfile}_imp
...
if [ -f ${finaloutfile}/genotypes_imp_chip0.txt ]; then
 ...
 rm ids.txt geno
 if [ $Allelecode = 12 ]; then
  ./plink --silent --cow --noweb --nonfounders --file file --make-bed --out ${finaloutfile}_ungenoimp
  ./plink --silent --cow --noweb --nonfounders --bfile ${finaloutfile}_ungenoimp --make-bed --out ${finaloutfile}_ungenoimp
 rm file.map file.ped
 elif [ $Allelecode = AB ]; then
  cat ../${ref}.bim | awk '{print $2,1,2,"A","B"}' > alleleupdate.txt
  ./plink --silent --cow --noweb --nonfounders --file file --update-alleles alleleupdate.txt --make-bed --out ${finaloutfile}_ungenoimp
  ./plink --silent --cow --noweb --nonfounders --bfile ${finaloutfile}_ungenoimp --make-bed --out ${finaloutfile}_ungenoimp
 rm alleleupdate.txt file.map file.ped
fi
fi
